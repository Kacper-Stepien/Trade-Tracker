...
import { STATS_RANGE, StatsRange } from "../types/Stats";
import {
...
export default function StatisticsPage() {
  ...
  const pageData = stats && charts ? { stats, charts } : null;

  return (
    <AsyncStateBoundary
      data={pageData}
      isLoading={isStatsLoading || isChartsLoading}
      isError={isStatsError || isChartsError}
      errorMessage={t("common.errors.fetchFailed")}
    >
      {(resolvedData) => {
        const { stats: resolvedStats, charts: resolvedCharts } = resolvedData;
        const totalProfitColor =
          resolvedStats.totalProfit >= 0
            ? PRODUCT_STATUS_COLORS.sold
            : PRODUCT_STATUS_COLORS.loss;
        const averageProfitPerSoldProduct =
          resolvedStats.numberOfSoldProducts > 0
            ? resolvedStats.soldProductsProfit / resolvedStats.numberOfSoldProducts
            : null;
        const sellThroughRate =
          resolvedStats.numberOfProducts > 0
            ? (resolvedStats.numberOfSoldProducts / resolvedStats.numberOfProducts) *
              100
            : null;
        const frozenCapital = resolvedStats.totalCosts - resolvedStats.soldProductsCosts;
        const avgDaysRounded =
          resolvedStats.averageDaysFromPurchaseToSale !== null
            ? Math.round(resolvedStats.averageDaysFromPurchaseToSale)
            : null;
          ...
          <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
            ...
            >
              <KpiCard
                ...
                value={formatPrice(resolvedStats.totalRevenue, i18n.language)}
              />
              <KpiCard
                size="large"
                label={t("pages.statistics.kpi.totalCosts")}
                value={formatPrice(resolvedStats.totalCosts, i18n.language)}
              />
              <KpiCard
                size="large"
                label={t("pages.statistics.kpi.netProfit")}
                value={formatPrice(resolvedStats.totalProfit, i18n.language)}
                accentColor={totalProfitColor}
                subtitle={formatPercentage(
                  resolvedStats.totalProfitPercentage,
                  i18n.language,
                )}
              />
              <KpiCard
                ...
                value={resolvedStats.numberOfSoldProducts}
              />
            </Box>
            ...
            >
              <KpiCard
                ...
                value={formatPercentage(
                  resolvedStats.soldProductsProfitPercentage,
                  i18n.language,
                )}
              />
              <KpiCard
                ...
                {chartPieData.length ? (
                  ...
                  >
                      ...
                      series={[
                        {
                          ...
                          valueFormatter: (value) =>
                            formatPrice(value.value, i18n.language),
                        },
                      ]}
                    ...
                    <Stack spacing={1.2} sx={{ width: "100%" }}>
                      {resolvedCharts.profitByCategory.map((item) => (
                        <Box
                          key={item.category}
                          sx={{
                            display: "flex",
